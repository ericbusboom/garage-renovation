...
(OUT/'south-framing-validation.json').write_text(json.dumps({'source_sha256':hashlib.sha256((P/'frame.compas.json').read_bytes()).hexdigest(),'view':'south looking north; west left, east right','frame_members':len(elements),'foreground_members':len(north),'background_members':len(background),'enclosure_objects':0},indent=2))
