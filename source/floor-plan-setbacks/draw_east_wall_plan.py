...
pdf=canvas.Canvas(str(OUT/'east-wall-floor-plan.pdf'),pagesize=(PW,PH))
...
text(60,65,'EAST WALL STUDY / FLOOR PLAN',28)
text(60,98,'EW-P6   Wall strengthening over the revision 11 plan • north up',16)
text(60,126,'Discussion sketch  |  September 15, 2026',13,label)
text(60,154,'ARCH-006 / P2  |  Rev 3  |  2026-09-15  |  Status: draft  |  Units: ft / in  |  Not to scale; use dimensions',12,label)
...
text(870,204,'EAST WALL FRAME / TWO OPTIONS',18,purple)
...
svg.append('</svg>');(OUT/'east-wall-floor-plan.svg').write_text('\n'.join(svg))
pdf.showPage();pdf.save();im.save(OUT/'east-wall-floor-plan.png')
...
(OUT/'east-wall-floor-plan-basis.json').write_text(json.dumps(basis,indent=2)+'\n')
...
print('Created east-wall companion plan: SVG, PNG, PDF and basis.')
