...
pdf=canvas.Canvas(str(OUT/'floor-plan-setbacks.pdf'),pagesize=(PW,PH))
...
text(60,65,'GARAGE / UPPER STRUCTURE + COLUMNS',28)
text(60,98,'11   Earlier floor-plan base • north up • dimensions in feet and inches',16)
text(60,126,'Discussion sketch  |  September 15, 2026',13,label)
text(60,154,'ARCH-006 / P1  |  Rev 3  |  2026-09-15  |  Status: draft  |  Units: ft / in  |  Not to scale; use dimensions',12,label)
...
text(870,205,'OUTLINES + SUPPORTS',17)
notes=[('Blue / full upper structure',blue,['West and south follow the outer beams.','East outside face: 4 ft from property line.','North outside face: 5 ft from property line.','Dashed blue: 2 ft 3 in north roof overhang.']),('Teal / loft floor',teal,['Expanded to the full available width.','West, east and north edges meet blue outline.','South edge follows W2 at Y = 69¾ in.','Supported along the west by W2, W3, W4.','Teal dashes overlay shared blue boundaries.']),('Columns / flush outside edges',ink,['W1 sits flush in the southwest corner.','S1 and S2 south faces meet the blue edge.','S2 east face also meets the blue edge.','S3 center aligned with east-wall post E-S.']),('Outer beam locations',blue,['West beam centerline: 32 in west of wall.','South beam supported by W1, S1 and S2.','Roof edge at beam outside face (assumed).'])]
y=244
for title,c,ls in notes:
    line(870,y-5,901,y-5,c,2);text(913,y,title,15)
    for st in ls:y+=24;text(870,y,st,13)
    y+=34
text(870,882,'COORDINATION NOTES',14)
for i,st in enumerate(['Blue outline retained; column symbols are 4 in.','W1/S1/S2 centers: 66 in south; faces at 68 in.','WBN removed; N1–N2 centers spaced 16 ft.','North column faces remain flush with 5-ft limit.']):text(870,908+i*22,st,12)
text(60,1020,'Sources: labeled framing plan / member register; earlier model for member widths; loft and north supports revised to your instructions.',12,label)
...
svg.append('</svg>');(OUT/'floor-plan-setbacks.svg').write_text('\n'.join(svg))
pdf.showPage();pdf.save();im.save(OUT/'floor-plan-setbacks.png')
...
print('Created revised SVG, PNG, PDF, and dimension basis with columns.')
