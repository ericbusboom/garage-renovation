...
for m in cad['members']:
 if m['id']=='T-E' or m.get('truss')=='TE' or m['id'].startswith('BR-E-upper'):
  for side in ['a','b']:
   if m[side][2]<130:m[side][2]+=(4 if m['role']=='vertical' else 5)
...
cad['candidate_revision']='TE lower axis +5in, 4x4x1/2 HSS; north ground braces +4in north for eave clearance; loft and roof heights retained. Joint solid distances not recomputed.'
sel['T-W / B-WO']='HSS4x4x0.25'
a.CAD=cad
(P/'candidate9-candidate-geometry.json').write_text(json.dumps(cad,indent=2));(P/'candidate9-candidate-selection.json').write_text(json.dumps(sel,indent=2))
for name,kw in [('candidate9_complete',{'second_order':True}),('candidate9_roof_first',{'study_stage':'roof_first'}),('candidate9_base_pinned',{'base_pinned':True})]:
