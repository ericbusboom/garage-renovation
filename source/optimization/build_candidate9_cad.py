...
cad=json.loads((P/'candidate9-candidate-geometry.json').read_text());sel=json.loads((P/'candidate9-candidate-selection.json').read_text());cat=json.loads((P/'iteration2-catalog.json').read_text());result=json.loads((P/'candidate9_complete.json').read_text())
G.showMainWindow();d=A.newDocument('GarageCandidate9');root=d.addObject('App::DocumentObjectGroup','StructuralCandidate');root.Label='Candidate 9 — preliminary; joints, bracing and foundations require design';groups={};mesh=[];count=0
...
o=d.addObject('Part::Box','Deck');o.Label='3/4-inch loft deck — connections not detailed';o.Length=222.25*I;o.Width=177*I;o.Height=.75*I;o.Placement.Base=A.Vector(0,72*I,120*I);loft.addObject(o);o.ViewObject.ShapeColor=(.83,.74,.57);o.ViewObject.Transparency=65
joists=d.addObject('App::DocumentObjectGroup','WoodJoists');root.addObject(joists)
for n,x in enumerate([.75+16*i for i in range(14)]+[221.5]):
...
d.recompute();G.activeDocument().activeView().viewAxonometric();G.activeDocument().activeView().fitAll();d.recompute();d.saveAs(str(P/'Garage-Candidate-9.FCStd'))
(P/'candidate9-cad-mesh.json').write_text(json.dumps({'units':'in','objects':mesh,'removed_columns':['W2'],'stage':'proposed','roof_lines':rooflines},indent=2))
(P/'candidate9-cad-validation.json').write_text(json.dumps({'valid_members':count,'invalid_members':0,'source_model_unchanged':True,'candidate_path':str(P/'Garage-Candidate-9.FCStd')},indent=2))
