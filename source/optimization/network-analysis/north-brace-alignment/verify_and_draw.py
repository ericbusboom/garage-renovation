 ...
 for case,ref in d['reference'].items():
  ...
  for n,u in ref['reactions'].items():
   for key,v in zip(['RxnFX','RxnFY','RxnFZ','RxnMX','RxnMY','RxnMZ'],u):
    ...
    scaled=max(scaled,err/(.1+1e-4*abs(v)))
 print('Replay discrepancy',du,dr,worst,'scaled',scaled,flush=True)
 assert du<1e-5 and scaled<1,(du,dr,worst,scaled)
 return dict(cases=len(d['combinations']),max_displacement_error=du,max_reaction_error_lb_or_lb_in=float(dr),worst_reaction_component=worst,max_normalized_error=float(scaled),reaction_tolerance='0.1 absolute + 0.0001 relative; force lb, moment lb-in')
...
def draw(m,d,group,cols,extras=(),iso=False):
 ...
 if group=='TN':
  ...
  detail.update(reset_camera=False,off_screen=True);dp=detail.plotter;dp.camera_position=[[104.25,215.625,1249],[104.25,215.625,249],[0,1,0]];dp.enable_parallel_projection();dp.camera.parallel_scale=32
