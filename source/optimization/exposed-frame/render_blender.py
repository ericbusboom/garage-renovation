...
colors={'steel':('354247',.48,.34),'panel':('b6c0c2',.3,.42),'trim':('ecebe4',.08,.38),'roof':('606d70',.52,.4),'pv':('243d50',.62,.18),'glass':('526f78',.35,.16),'wood':('aa865c',0,.62),'stucco':('b7b9b3',0,.83),'slab':('a6a7a2',0,.84)}
...
cameras=[(cam('Southwest exterior',(-11,-13,9),(2.0,1.8,3),48),'01-southwest'),(cam('Northwest exterior',(-10.5,17,8.5),(2.1,3.6,3.2),48),'02-northwest')]
scene.render.engine='CYCLES';scene.cycles.device='CPU';scene.cycles.samples=256;scene.cycles.use_denoising=True;scene.cycles.adaptive_threshold=.01;scene.render.resolution_x=1800;scene.render.resolution_y=1500;scene.render.resolution_percentage=100;scene.view_settings.view_transform='AgX';scene.view_settings.exposure=0;scene.view_settings.look='AgX - Medium High Contrast';scene.render.image_settings.file_format='PNG';scene.camera=cameras[0][0]
scene['design']='Weight-sized fixed frame; 2-in illustrative insulated metal panels placed inside exposed frame; existing hip cap retained.';scene['engineering_limit']='Connection/secondary framing and envelope attachment not designed. This is an architectural assembly visualization.'
bpy.ops.wm.save_as_mainfile(filepath=str(P/'Garage-Exposed-Steel.blend'))
...
(P/'render-validation.json').write_text(json.dumps(dict(source_objects=len(d['objects']),render_engine='Cycles CPU',samples=256,resolution=[1800,1500],cameras=[c.name for c,_ in cameras],source='Exact CAD tessellation in metres; bevel finish only'),indent=2))
