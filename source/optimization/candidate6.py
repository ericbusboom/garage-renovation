...
a.CAD=cad
(P/'candidate6-candidate-geometry.json').write_text(json.dumps(cad,indent=2));(P/'candidate6-candidate-selection.json').write_text(json.dumps(sel,indent=2))
for name,kw in [('candidate6_complete',{'second_order':True}),('candidate6_roof_first',{'study_stage':'roof_first'}),('candidate6_base_pinned',{'base_pinned':True})]:
 r=a.analyze(name,remove=['W2'],selection=sel,catalog=CAT,demand_export=True,lateral_sensitivity=True,link_factor=100,**kw)
 if r['status']=='solved_conditional':
  r['selection']=sel;r['member_checks']=screen(r,sel);r['max_screen_ratio']=max(v['ratio'] for v in r['member_checks'].values());r['storage125_sensitivity_ratio']=max(v['ratio'] for v in screen(r,sel,include_storage=True).values())
