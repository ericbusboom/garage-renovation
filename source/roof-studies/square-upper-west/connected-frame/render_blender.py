...
for key,(hx,metal,rough) in colors.items():
 ...
 mats[key]=m
cols={};d=json.loads((P/'scene-mesh.json').read_text())
for e in d['objects']:
 g='Garage / '+e['group']
 ...
 ob=bpy.data.objects.new(e['name'],mesh);cols[g].objects.link(ob);ob.data.materials.append(mats[e['material']]);ob['source']='frame-spec.json / COMPAS Graph + Model'
...
scene['design']='Rough roof concept: east setback infill, retained hip cap, six vertical clerestory panes and extended hip cap. Shared COMPAS joint graph; geometric connectivity checked.';scene['engineering_limit']='Connection/secondary framing and envelope attachment not designed. This is an architectural assembly visualization.'
...
(P/'render-validation.json').write_text(json.dumps(dict(source_objects=len(d['objects']),render_engine='Cycles CPU',samples=16,resolution=[1100,900],cameras=[c.name for c,_ in cameras],source='Exact COMPAS scene geometry in metres; bevel finish only'),indent=2))
