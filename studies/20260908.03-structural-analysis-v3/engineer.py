...
sel={f:old[f.replace(':upper',':chord').replace(':lower',':chord')]['name'] for f in fams}
for f in ['B2:beam','T1:lower']:sel[f]='W8X31'
...
def allowed(f):
 return sorted([s for s in sections if (s['type']=='W' if f in ['B2:beam','T1:lower'] else s['type']=='HSS') and (s['d']==4 if f=='columns' else True)],key=lambda s:s['w'])
