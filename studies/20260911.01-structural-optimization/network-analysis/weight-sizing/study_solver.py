...
def analyze(name,remove=(),web_pinned=False,base_pinned=False,link_factor=1000,second_order=False,scale=1,diagnostic=False,selection=None,catalog=None,demand_export=False,study_stage="complete",lateral_sensitivity=False,export_hook=None,export_basic_cases=False):
 ...
 for lo,hi,q,surfacefactor in [(-63,br,BASIS['roof']['main_dead_psf_surface'],1/math.cos(math.pi/6)),(br,271,BASIS['roof']['cap_dead_psf_surface'],math.sqrt(1+(18/half)**2))]:
  ...
  for x0,x1 in zip(xx[:-1],xx[1:]):
   for y0,y1 in zip(yy[:-1],yy[1:]):
    ...
    # Roof sheeting transfers to explicit E-W purlin stations, then TE/TW.
    # Linear tributary splitting preserves the original roof force and first moment.
    stations=[-63.,-15.,33.,81.,br,br+48,br+96,253.]
    if y>=stations[-1]:reactions=[(stations[-1],1.,y-stations[-1])]
    else:
     j=next(j for j in range(len(stations)-1) if stations[j]<=y<=stations[j+1]);t_y=(y-stations[j])/(stations[j+1]-stations[j]);reactions=[(stations[j],1-t_y,0.),(stations[j+1],t_y,0.)]
    for tr,f in [('TW',1-t),('TE',t)]:
     for station,share,ecc in reactions:
      e,p=top(tr,station)
      for case,psf in [('Droof',q*surfacefactor),('R20',BASIS['roof']['live_psf_plan']),('U20',-20)]:
       wt=psf*area*f*share;put(e['id'],p,case,-wt,ecc*wt)
 ...
 # Explicit west balcony reserve, outside the main loft area.
 if study_stage=='complete':
  for y in np.linspace(186+2.5,246-2.5,12):
   area=32*5/144
   for case,q in [('Dfloor',12),('Lbalcony',100)]:
    wt=q*area;put('T-W / B-WO',[-32,y,115],case,-wt,mz=-16*wt)
 ...
 if study_stage=='complete':
  combos['strength_machine']={**{k:1.2 for k in D},'L75patch':1.6,'R20':.5,'P200':1.6}
  combos['patches_live_only']={'L75patch':1,'Lbalcony':1}
  combos['dead_only']=D.copy()
  for k,factors in combos.items():
   if k not in ['dead_only','live_only','uplift20_sensitivity'] and not k.startswith('basic_'):
    factors.setdefault('Lbalcony',1.6 if k.startswith('strength') or k=='storage125_strength_sensitivity' else 1.)
 # Uplift reverses chord actions; retain it as an explicit strength sensitivity.
 combos['strength_uplift_sensitivity']={'Dsteel':.9,'Droof':.9,'U20':1}
 if study_stage=='complete':combos['strength_uplift_sensitivity'].update(Dfloor=.9,Dwall=.9)
