...
r=json.loads((P/'candidate8_complete.json').read_text());stage=json.loads((P/'candidate8_roof_first.json').read_text());pin=json.loads((P/'candidate8_base_pinned.json').read_text());c=json.loads((P/'candidate8-candidate-geometry.json').read_text());mesh=json.loads((P/'candidate8-cad-mesh.json').read_text());sel=r['selection'];cat=json.loads((P/'iteration2-catalog.json').read_text());mm={m['id']:m for m in c['members']}
...
fig.suptitle('Candidate 8 | Braced steel frame and renovated loft',fontsize=20)
...
fig.savefig(P/'candidate8-overview.png',dpi=150)
...
fig2.savefig(P/'candidate8-comparison.png',dpi=150)
# Every physical member has reproducible strength demand and capacity data.
with (P/'candidate8-member-checks.csv').open('w') as f:
...
with (P/'candidate8-foundation-reactions.csv').open('w') as f:
...
(P/'candidate8-secondary-framing.json').write_text(json.dumps(checks,indent=2))
clearfile=P/'candidate8-clearance-check.json';clear=json.loads(clearfile.read_text()) if clearfile.exists() else {}
...
text=f'''# Candidate 8 — garage structural study
...
The candidate is saved separately as Garage-Candidate-8.FCStd. The original Proposed-Garage.FCStd remains unchanged. CAD was reopened and all {clear.get('reopened_valid_members','pending')} candidate steel solids validated. The existing roof is unclad in that model; field clearances and cladding thickness still matter.
...
'''
(P/'CANDIDATE-8-RESULTS.md').write_text(text)
with PdfPages(P/'Garage-Candidate-8-Results.pdf') as pdf:
