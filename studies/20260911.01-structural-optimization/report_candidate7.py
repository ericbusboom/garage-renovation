...
r=json.loads((P/'candidate7_complete.json').read_text());stage=json.loads((P/'candidate7_roof_first.json').read_text());pin=json.loads((P/'candidate7_base_pinned.json').read_text());c=json.loads((P/'candidate7-candidate-geometry.json').read_text());mesh=json.loads((P/'candidate7-cad-mesh.json').read_text());sel=r['selection'];cat=json.loads((P/'iteration2-catalog.json').read_text());mm={m['id']:m for m in c['members']}
...
for m in c['members']:
 ...
 if m['id'].startswith('BR-') or m['section_family'] in ['chord','Wbeam']:plan.plot([a[0],b[0]],[a[1],b[1]],color=col,lw=2 if m['id'].startswith('BR-') else .9,alpha=.9)
...
deck=np.array([[0,72,120],[224.25,72,120],[224.25,249,120],[0,249,120]])
ax.add_collection3d(Poly3DCollection([deck],facecolors='#dcc49b',alpha=.35))
plan.add_patch(Rectangle((0,72),224.25,177,facecolor='#ead9ba',alpha=.35,zorder=0))
...
fig.suptitle('Candidate 7 | Braced steel frame and renovated loft',fontsize=20)
...
fig.savefig(P/'candidate7-overview.png',dpi=150)
...
fig2.savefig(P/'candidate7-comparison.png',dpi=150)
# Every physical member has reproducible strength demand and capacity data.
with (P/'candidate7-member-checks.csv').open('w') as f:
...
with (P/'candidate7-foundation-reactions.csv').open('w') as f:
...
(P/'candidate7-secondary-framing.json').write_text(json.dumps(checks,indent=2))
clearfile=P/'candidate7-clearance-check.json';clear=json.loads(clearfile.read_text()) if clearfile.exists() else {}
...
text=f'''# Candidate 7 — garage structural study
...
- TE upper chord: HSS 6×6×1/4. TE lower chord: HSS 4×4×1/2, centerline raised from 115 to 117 in to clear old rafters while retaining the floor and roof elevations.
...
Files include the Python model, stock catalog, geometry, load basis, member checks and every case reaction. Run candidate7.py with PyNiteFEA 3.0.0 after installing requirements.txt to repeat this candidate from the supplied prior selection input. Change design-basis.json for loads, re-export geometry when CAD changes, and compare all cases before accepting a revision. The historical trials have different scope/assumptions; candidate7 results are the current reference.
...
'''
(P/'CANDIDATE-7-RESULTS.md').write_text(text)
with PdfPages(P/'Garage-Candidate-7-Results.pdf') as pdf:
