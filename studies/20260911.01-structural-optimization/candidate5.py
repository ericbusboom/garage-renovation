...
a.CAD=cad
(P/'candidate5-candidate-geometry.json').write_text(json.dumps(cad,indent=2));(P/'candidate5-candidate-selection.json').write_text(json.dumps(sel,indent=2))
for name,kw in [('candidate5_complete',{'second_order':True}),('candidate5_roof_first',{'study_stage':'roof_first'}),('candidate5_base_pinned',{'base_pinned':True})]:
 r=a.analyze(name,remove=['W2'],selection=sel,catalog=CAT,demand_export=True,lateral_sensitivity=True,**kw)
