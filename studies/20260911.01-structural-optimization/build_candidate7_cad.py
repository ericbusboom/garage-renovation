...
cad=json.loads((P/'candidate7-candidate-geometry.json').read_text());sel=json.loads((P/'candidate7-candidate-selection.json').read_text());cat=json.loads((P/'iteration2-catalog.json').read_text());result=json.loads((P/'candidate7_complete.json').read_text())
G.showMainWindow();d=A.newDocument('GarageCandidate7');root=d.addObject('App::DocumentObjectGroup','StructuralCandidate');root.Label='Candidate 7 — preliminary; joints, bracing and foundations require design';groups={};mesh=[];count=0
...
d.recompute();G.activeDocument().activeView().viewAxonometric();G.activeDocument().activeView().fitAll();d.recompute();d.saveAs(str(P/'Garage-Candidate-7.FCStd'))
(P/'candidate7-cad-mesh.json').write_text(json.dumps({'units':'in','objects':mesh,'removed_columns':['W2'],'stage':'proposed'},indent=2))
(P/'candidate7-cad-validation.json').write_text(json.dumps({'valid_members':count,'invalid_members':0,'source_model_unchanged':True,'candidate_path':str(P/'Garage-Candidate-7.FCStd')},indent=2))
