...
new=A.openDocument(str(P/'Garage-Candidate-5.FCStd'));old=A.openDocument(str(P.parent/'Existing-Garage.FCStd'))
...
r={'reopened_valid_members':len(parts),'existing_solids_compared':len(oldparts),'roof_first_solid_intersections':clashes,'limitations':'Existing roof cladding is not modeled; erection tolerances and access are not checked. Intersections at columns may require planned local openings; these are not automatically acceptable.'}
(P/'candidate5-clearance-check.json').write_text(json.dumps(r,indent=2));print(json.dumps(r),flush=True)
os._exit(0)
