...
# Owner revision 2026-09-17 (2): the east interior wall is removed entirely — the
# east setback roof and its cladding enclose that side, so it was only a partition.
# North cladding starts at the T-N bottom chord rather than running down into the
# garage door bay; the ground wall below it rises to meet the same line.
TN_CHORD_UNDERSIDE=115.0-1.5   # T-N centerline bottom_z 115, 3 in deep
GARAGE_DOOR_HEAD=84.0          # 7 ft clear opening
GARAGE_HEADER_TOP=GARAGE_DOOR_HEAD+1.5
N1_EAST_FACE=40.34769094138544+3.0
N2_WEST_FACE=209.5
...
for o in list(d.Objects):
 ...
 if label.startswith(('North ','West glazing','West opening','E inside metal panel')):
 ...
 if label.startswith('N inside metal panel'):
  ...
  # Keep the cladding within the truss: it starts at the T-N bottom chord and
  # no longer runs down into the garage door bay below.
  shape=shape.common(Part.makeBox(2000*U,2000*U,(1000-TN_CHORD_UNDERSIDE)*U,
                                  A.Vector(-1000*U,-1000*U,TN_CHORD_UNDERSIDE*U)))
...
wall_specs=[('Extension wall W3 to W4',wall_x,w3['y'],0,wall_t,NORTH_FACE-w3['y'],98.5),
            ...
            ('North ground wall W4 to N1',wall_x,NORTH_WALL_FACE-north_ground_t,0,
             (N1_X-N1_HALF)-wall_x,north_ground_t,TN_CHORD_UNDERSIDE),
            ...
            # Infill behind the garage-door truss: from the door head up to the
            # T-N bottom chord, where the north cladding takes over. Held between
            # the post faces so it clears N1 and N2.
            ('North wall over garage door',N1_EAST_FACE,NORTH_WALL_FACE-north_ground_t,
             GARAGE_DOOR_HEAD,N2_WEST_FACE-N1_EAST_FACE,north_ground_t,
             TN_CHORD_UNDERSIDE-GARAGE_DOOR_HEAD)]
...
removed_west=['T-W vertical 1','West revised diagonal south','T-W balcony header','T-W vertical 13','T-W diagonal 15','T-W diagonal 16',
              'T-W top / segment 1','T-W top / segment 2','T-W vertical 6','T-W vertical 3','T-W vertical 4','T-W diagonal 9','T-W diagonal 10']
for o in list(d.Objects):
...
for o in d.Objects:
 ...
 if any(g.Name=='Columns' for g in o.InList):
  if o.Label.startswith(('W3 |','W4 |')):
   o.Shape=o.Shape.common(Part.makeBox(100*U,400*U,(glazing_head-5)*U,V((-80,-100,0))))
  else:o.Shape=o.Shape.common(clip)
  trimmed_posts.append(o.Label)
# Extend the upper west wall into a rectangular bay ending at the clerestory front.
flat_top=glazing_head;flat_bottom=flat_top-5
beam('West square upper chord',[-34,front,flat_top-2.5],[-34,259,flat_top-2.5],5,5,'WestMarkupFraming','steel')
beam('West solar slope chord',[-34,-63,117],[-34,front,117+(front+63)*m],2.5,2.5,'WestMarkupFraming','steel')
# Fill between original west wall profile and the new flat chord, matching inboard wall cladding.
pts=[(wall_x,front,120+(front+63)*m),(wall_x,br,227.25),
     (wall_x,261,227.25),(wall_x,261,flat_bottom),(wall_x,front,flat_bottom)]
# Rectangular infill from a constant lower datum avoids a self-intersecting transition polygon.
fill=d.addObject('PartDesign::Feature','SquareWestWallInfill');fill.Label='Square upper west wall infill'
fill.Shape=Part.makeBox(wall_t*U,(261-front)*U,(flat_bottom-190)*U,V((wall_x,front,190)))
d.getObject('InteriorMetalPanels').addObject(fill);fill.ViewObject.ShapeColor=(.68,.72,.73)
# One diagonal retained at rear; the other moves into the new rectangular bay.
for name,a,b,ymin,ymax in [('West rear diagonal rising north',[-34,185,116.75],[-34,259,flat_bottom],185,259),
                          ('West square bay diagonal',[-34,185,116.75],[-34,front,flat_bottom],front,185)]:
 ob=beam(name,a,b,2.25,2.25,'WestMarkupFraming','steel')
 ob.Shape=ob.Shape.common(Part.makeBox(100*U,(ymax-ymin)*U,(flat_bottom-116.75)*U,V((-80,ymin,116.75))))
beam('West clerestory-aligned vertical',[-34,front,116.75],[-34,front,flat_bottom],4,4,'WestMarkupFraming','steel')
# Move the connected diagonal apex with the vertical, retaining the other end stations.
meet=117+(front+63)*m


markup_record={'removed_members':removed_west,'added_upper_bay_x':False,'brace_choice':'retain rear north-rising diagonal; move south-rising diagonal into new square bay','clerestory_post_section_in':[4,4],'clerestory_vertical_y':front,
 'clerestory_vertical_top_z':flat_bottom,'square_upper_chord_top_z':flat_top,'trimmed_posts':trimmed_posts,
...
# T-E is generated from the current T-W solids, not a separate older truss recipe.
east_group=d.addObject('App::DocumentObjectGroup','MatchedEastTruss')
pairs=[]
upper_zone=Part.makeBox(2000*U,1000*U,1000*U,V((-1000,-100,113.25)))
for source in list(d.Objects):
 if 'Shape' not in source.PropertiesList or source.Shape.isNull():continue
 is_west=any(g.Name in ('Truss_TW','WestMarkupFraming') for g in source.InList)
 is_post=any(g.Name=='Columns' for g in source.InList) and source.Shape.BoundBox.XMax/U<-25
 if not(is_west or is_post):continue
 sh=source.Shape.copy() if is_west else source.Shape.common(upper_zone)
 if sh.isNull() or sh.Volume<1e-6:continue
 # Reflect at the midplane between X=-34 and X211.5.
 mirror=A.Matrix();mirror.A11=-1;mirror.A14=(east+west)*U
 sh=sh.transformGeometry(mirror)
 ob=d.addObject('PartDesign::Feature','MatchedEastMember');ob.Label='T-E matching '+source.Label;ob.Shape=sh
 east_group.addObject(ob);ob.ViewObject.ShapeColor=colors['steel']
 ob.addProperty('App::PropertyString','StudyMaterial');ob.StudyMaterial='steel'
 ob.addProperty('App::PropertyString','MirroredFrom');ob.MirroredFrom=source.Label
 assert sh.isValid() and abs(sh.Volume-(source.Shape.Volume if is_west else source.Shape.common(upper_zone).Volume))<.01
 pairs.append({'west':source.Label,'east':ob.Label,'upper_post_only':is_post})
# Coordinate the transverse framing with the current side trusses.
# The inherited short transverse span ended at the old inner east row.
old_cross_east=-34+(224.25+32)*((211.5+34)/(249.5+32))
cross_scale=(east-west)/(old_cross_east-west)
transverse_updates=[]
for ob in list(d.Objects):
 if 'Shape' not in ob.PropertiesList or ob.Shape.isNull():continue
 group=next((g.Name for g in ob.InList if g.TypeId=='App::DocumentObjectGroup'),'')
 if group not in ('Truss_TS','Truss_T1','LoftSteel'):continue
 transform=A.Matrix();transform.A11=cross_scale;transform.A14=west*(1-cross_scale)*U
 # T1 meets the moved clerestory posts at both ends; its floor beam and hangers follow.
 moved_t1=group=='Truss_T1' or (group=='LoftSteel' and ob.Label.startswith('T1 '))
 if moved_t1:transform.A24=(front-69.75)*U
 ob.Shape=ob.Shape.transformGeometry(transform)
 assert ob.Shape.isValid(),ob.Label
 transverse_updates.append({'member':ob.Label,'east_axis_x':east,'moved_to_clerestory_station':moved_t1})
# Solid-distance checks catch a floating end even when bounding boxes overlap.
west_shapes=[ob.Shape for ob in d.Objects if 'Shape' in ob.PropertiesList and not ob.Shape.isNull() and
 (any(g.Name in ('Truss_TW','WestMarkupFraming') for g in ob.InList) or
  (any(g.Name=='Columns' for g in ob.InList) and ob.Shape.BoundBox.XMax/U<-25))]
west_solid=Part.makeCompound(west_shapes);east_solid=Part.makeCompound([ob.Shape for ob in east_group.Group])
connection_checks=[]
for ob in d.Objects:
 if 'Shape' not in ob.PropertiesList or ob.Shape.isNull():continue
 if not ob.Label.startswith(('T-S |','T-S top /','T1 upper lower chord','T1 top /','T1 future floor beam','B2 future floor beam','T-SO |','T-N |','T-N top /')):continue
 gaps={side:ob.Shape.distToShape(target)[0]/U for side,target in [('west',west_solid),('east',east_solid)]}
 assert max(gaps.values())<.01,(ob.Label,gaps)
 connection_checks.append({'member':ob.Label,'gap_inches':gaps})
assert len(connection_checks)==9,len(connection_checks)
(P/'crossbeam-connections.json').write_text(json.dumps({'old_east_endpoint_x':old_cross_east,'new_east_endpoint_x':east,
 't1_old_y':69.75,'t1_new_y':front,'updates':transverse_updates,'solid_contact_checks':connection_checks,
 'scope':'Geometric contact only; joints, welds, bolts, bearing and capacity are not designed.'},indent=2))
# Hide conflicting upper lengths of older east support context; lower supports stay as context.
for ob in list(d.Objects):
 if 'Shape' in ob.PropertiesList and any(g.Name=='Columns' for g in ob.InList) and ob.Label.startswith(('S2 |','S3 |','N2 |')):
  ob.Shape=ob.Shape.common(Part.makeBox(2000*U,1000*U,113.25*U,V((-1000,-100,0))))
(P/'matching-trusses.json').write_text(json.dumps({'west_axis_x':west,'east_axis_x':east,'pairs':pairs,'method':'exact reflected solids; upper post portions included','geometry_checks':'valid solids and mirrored volume equality'},indent=2))
def chord(y):
 return 117+(y+63)*math.tan(math.pi/6) if y<front else flat_top-2.5
top=[{'y1':front}]
...
d.saveAs(str(P/'garage-square-upper-west.FCStd'))
objects=[];export=[]
for o in d.Objects:
 if 'Shape' not in o.PropertiesList or o.Shape.isNull():continue
